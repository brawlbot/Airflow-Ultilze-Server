...
with dag:
    ...
    check_env_task = BashOperator(
        ...
        bash_command=f"source /opt/conda/etc/profile.d/conda.sh && conda activate pyspark-airflow && conda env export > /tmp/environment.yml && cat /tmp/environment.yml && tail -f /dev/null",
        dag=dag,
    )
